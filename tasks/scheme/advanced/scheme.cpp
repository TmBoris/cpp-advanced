...
std::variant<bool, int64_t, std::shared_ptr<Object>> SymbolEval(std::shared_ptr<Object> obj,
                                                                std::shared_ptr<Object> second, Scope) {
    ...
    if (command == "number?") {
        ...
        if (!second) {
            throw RuntimeError("nothing after symbol?");
        } else if (Is<Cell>(second) && Is<Cell>(As<Cell>(second)->first_) && Is<Symbol>(As<Cell>(As<Cell>(second)->first_)->first_) && As<Symbol>(As<Cell>(As<Cell>(second)->first_)->first_)->str_ == "quote") {
            return true;
        } else {
            return false;
        }
    } else if (command == "define")  {
        if (Is<Cell>(second) && Is<Symbol>(As<Cell>(second)->first_) && Is<Cell>(As<Cell>(second)->second_) && !As<Cell>(As<Cell>(second)->second_)->second_) {

        }
    }

    else {
        throw NameError("SymbolEval failed");
    }
}

void Eval(std::shared_ptr<Object> first, std::vector<std::shared_ptr<Object>>& ans, Scope* scope = nullptr) {
    if (Is<Number>(first)) {
        ans.push_back(first);
